#include "stdafx.h"
#include "..\Public\Player_LeftFeet.h"
#include "GameInstance.h"

CPlayer_LeftFeet::CPlayer_LeftFeet(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
	, m_eDir(DIRECTION::DIR_FRONT)
{
}

CPlayer_LeftFeet::CPlayer_LeftFeet(const CPlayer_LeftFeet & rhs)
	: CGameObject(rhs)
{
	m_eDir = rhs.m_eDir;
}

HRESULT CPlayer_LeftFeet::NativeConstruct_Prototype()
{
...
HRESULT CPlayer_LeftFeet::NativeConstruct(void * pArg)
{
...
void CPlayer_LeftFeet::Tick(_float fTimeDelta)
{
	...
	if ((vFeetPosition.x != vTempPosition.x) && (vFeetPosition.z == vTempPosition.z))	// 좌,우로 움직일 때의 발움직임
	{
		...
		D3DXMatrixRotationX(&worldMatrix, D3DXToRadian(fFeetRotate));
	...
	else if ((vFeetPosition.x == vTempPosition.x) && (vFeetPosition.z != vTempPosition.z))	// 상,하로 움직일 때의 발움직임
	{
		if (ObjectRotate == false)
		{
			fFeetRotate += 0.5f;
...
void CPlayer_LeftFeet::LateTick(_float fTimeDelta)
{
...
HRESULT CPlayer_LeftFeet::Render()
{
...
HRESULT CPlayer_LeftFeet::SetUp_Components(void* pArg)
{
	/* For.Com_Transform */
	...
	/* For.Com_VIBuffer */
	if (FAILED(__super::SetUp_Components(TEXT("Com_VIBuffer"), LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Object"), (CComponent**)&m_pVIBufferCom, TEXT("Frog_Left_Leg.dat"))))
		return E_FAIL;
...
HRESULT CPlayer_LeftFeet::SetUp_OnTerrain()
{
	CGameInstance*			pGameInstance = CGameInstance::GetInstance();
...
CPlayer_LeftFeet * CPlayer_LeftFeet::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CPlayer_LeftFeet*	pInstance = new CPlayer_LeftFeet(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CPlayer_LeftFeet"));
...
CGameObject * CPlayer_LeftFeet::Clone(void * pArg)
{
	CPlayer_LeftFeet*	pInstance = new CPlayer_LeftFeet(*this);
