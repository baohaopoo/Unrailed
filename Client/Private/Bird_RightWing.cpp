...
CBird_RightWing::CBird_RightWing(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
	m_eType = OBJECT_TYPE::OBJECT_BIRDRIGHTWING;
}

CBird_RightWing::CBird_RightWing(const CBird_RightWing & rhs)
	: CGameObject(rhs)
{
...
HRESULT CBird_RightWing::NativeConstruct_Prototype()
{
...
HRESULT CBird_RightWing::NativeConstruct(void * pArg)
{
...
void CBird_RightWing::Tick(_float fTimeDelta)
{
...
void CBird_RightWing::LateTick(_float fTimeDelta)
{
...
HRESULT CBird_RightWing::Render()
{
...
HRESULT CBird_RightWing::SetUp_Components(void* pArg)
{
	/* For.Com_Transform */
	...
	if (FAILED(__super::SetUp_Components(Component_VIBuffer, LEVEL_STATIC, Prototype_Compo_VIB_Object, (CComponent**)&m_pVIBufferCom, TEXT("Bird_RightWing.dat"))))
		return E_FAIL;
...
CBird_RightWing * CBird_RightWing::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CBird_RightWing*	pInstance = new CBird_RightWing(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created Bird_RightWing"));
...
CGameObject * CBird_RightWing::Clone(void * pArg)
{
	CBird_RightWing*	pInstance = new CBird_RightWing(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX(TEXT("Failed to Created Clone Bird_RightWing"));
		Safe_Release(pInstance);
	}
